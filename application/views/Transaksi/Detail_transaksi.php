            ...
            <form class="form-inline" method="post" action="<?= base_url('transaksi1/tambah_aksi'); ?>" enctype="multipart/form-data">
                ...
                <div class="form-group col-6">
                    ...
                    <select name="tahun_ajaran" id="tahun_ajaran" class="form-control" style="margin-left: 10px;">
                        <?php
                        foreach ($this->db->query('SELECT tahun_ajaran.id_tahun, tahun_ajaran.tahun_ajaran, tahun_ajaran.besar_spp FROM tahun_ajaran JOIN tahun_aktif ON tahun_ajaran.id_tahun=tahun_aktif.id_tahun WHERE nis=\'' . $u->nis . '\'')->result() as $tahun) { /*$this->m_transaksi->tampil_datatahun()->result() */
                        ?>
                        ...
                        ?>
                            <tr>
                                ...
                                <?php if ($u->id == $user['id']) { ?>
                                    ...
                                    <td>
                                        <?php echo anchor('transaksi1/hapus/' . $u->id_transaksi . '/' . $u->nis . '/' . $u->id_tahun, '<input type=reset class="btn btn-danger" value=\'Hapus\'>'); ?> <br>
