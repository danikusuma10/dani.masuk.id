...
class Datapembayaran_model extends CI_Model
{
	...
	function copy_input($where)
	{
		$this->db->query('INSERT INTO hapus_transaksi (id_transaksi,nis,id_bulan,id_tahun,tanggal_bayar,id)
                      ...
                      FROM datapembayaran WHERE id_transaksi = \'' . $where . '\'');
	}
	...
	{
		$query = $this->db->get('datapembayaran');
		if ($query->num_rows() > 0) {
	...
	{
		$this->db->select('*');
		$this->db->from('datapembayaran');
		$this->db->join('bulan', 'datapembayaran.id_bulan = bulan.id_bulan');
		$this->db->join('tahun_ajaran', 'datapembayaran.id_tahun = tahun_ajaran.id_tahun');
		$this->db->join('user', 'datapembayaran.id = user.id');
		$this->db->where_in('nis', $where1);
		$this->db->order_by('datapembayaran.id_tahun,datapembayaran.id_bulan', 'ASC');
		return $query = $this->db->get();
	}
	...
	{
		$q = $this->db->query("SELECT MAX(RIGHT(id_transaksi,3)) AS kd_max FROM datapembayaran WHERE DATE(tanggal_bayar)=CURDATE()");
		$kd = "";
		if ($q->num_rows() > 0) {
	...
	public function view_by_date($tanggal1, $tanggal2)
	{
		$this->db->select('*');
		$this->db->from('datapembayaran');
		$this->db->join('siswa', 'datapembayaran.nis = siswa.nis');
		$this->db->join('bulan', 'datapembayaran.id_bulan = bulan.id_bulan');
		$this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun = datapembayaran.id_tahun');
		$this->db->join('user', 'datapembayaran.id = user.id');
	...
	public function view_by_nis($nis)
	{
		...
		$this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun = datapembayaran.id_tahun');
		$this->db->join('user', 'datapembayaran.id = user.id');
		$this->db->where('datapembayaran.nis', $nis);
		$this->db->order_by('datapembayaran.id_tahun');
		return $query = $this->db->get(); // Tampilkan data transaksi sesuai tanggal yang diinput oleh user pada filter  
	}
	...
	public function view_by_year($tahun)
	{
		...
		$this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun = datapembayaran.id_tahun');
		$this->db->join('user', 'datapembayaran.id = user.id');
		$this->db->where('datapembayaran.id_tahun="' . $tahun . '"');
		$this->db->order_by('datapembayaran.id_tahun');
		return $query = $this->db->get();
	}
	...
	function view_all()
	{
		...
		$this->db->select('*');
		$this->db->from('datapembayaran');
		$this->db->join('siswa', 'siswa.nis = datapembayaran.nis');
		$this->db->join('bulan', 'datapembayaran.id_bulan = bulan.id_bulan');
		$this->db->join('tahun_ajaran', 'tahun_ajaran.id_tahun = datapembayaran.id_tahun');
		$this->db->join('user', 'datapembayaran.id = user.id');
